...
# TODO
# 1. Set up function to rotate and translate point based on some constraints
#    a. don't forget to clamp to int
# 2. iterate across all rotations (24) and translations
#    a. define translations as moving scanner 2's first point to each of scanner 1's
#       points, then comparing to see how many match
...
class Scanner:
    ...
    def rotate(self, yaw, pitch, roll):
        Rz = Scanner.__rotz(yaw)
        Ry = Scanner.__roty(pitch)
        Rx = Scanner.__rotx(roll)
        self._rotate = np.dot(np.dot(Rz, Ry), Rx)
        
        for i in range(0,len(self._backup)):
            self._rotated[i] = np.dot(self._rotate, self._backup[i])

    def translate(self, dr):
        self._translate = dr - self._rotated[0]
    ...
    def __eq__(self, other):
        ...
        for i in range(0,len(self.points)):
            for j in range(0,len(other.points)):
                if np.array_equal(self.points[i], other._backup[j]):
                    ct += 1
        
        return True if ct >= overlap else False

    def postTransform(self, other):
        for i in range(0,len(self._backup)):
            self._rotated[i] = np.dot(other._rotate, self.points[i])
            self.points[i] = self._rotated[i] + other._translate
        if other.name != 'scanner 0' and other.matched is not None:
            self.postTransform(other.matched)
    ...
    for yaw in rotations:
        for pitch in rotations:
            for roll in rotations:
                scanners[i].rotate(yaw,pitch,roll)

                for j in range(0,len(scanners)):
                    if i != j:
                        for pt in scanners[j]._backup:
                            scanners[i].translate(pt)

                            if scanners[i] == scanners[j]:
                                scanners[i].matched = scanners[j]
                                break
                        else:
                            continue
                        break
...
for i in range(1, len(scanners)):
    if scanners[i].matched is not None:
        scanners[i].postTransform(scanners[i].matched)

uniquepts = []
for scn in scanners:
    if scn.matched is not None:
