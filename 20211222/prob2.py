...
class Range:
    def __init__(self, pwr, xmin, xmax, ymin, ymax, zmin, zmax):
        self.on = pwr
        self.xmin = xmin
        self.xmax = xmax
        self.ymin = ymin
    ...
    def subrange(self, other):
        # Return the portion of other that overlaps with self, using self's on state
        if other.xmax < self.xmin or other.xmin > self.xmax:
            return None
        if other.ymax < self.ymin or other.ymin > self.ymax:
            return None
        if other.zmax < self.zmin or other.zmin > self.zmax:
            return None

        xmin = self.xmin if self.xmin >= other.xmin else other.xmin
        ymin = self.ymin if self.ymin >= other.ymin else other.ymin
        zmin = self.zmin if self.zmin >= other.zmin else other.zmin

        xmax = self.xmax if self.xmax <= other.xmax else other.xmax
        ymax = self.ymax if self.ymax <= other.ymax else other.ymax
        zmax = self.zmax if self.zmax <= other.zmax else other.zmax

        return Range(self.on, xmin, xmax, ymin, ymax, zmin, zmax)

    def __eq__(self, other):
        if self.xmin == other.xmin and self.xmax == other.xmax:
            if self.ymin == other.ymin and self.ymax == other.ymax:
                if self.zmin == other.zmin and self.zmax == other.zmax:
                    return True
        return False

    def subtract(self, other):
        # Remove the portion of other that overlaps with self and return the resulting subranges
        # (all of which are on)
        sr = self.subrange(other)
        if sr is None:
            return None

        sub = []
        xr = [self.xmin, sr.xmin, sr.xmax+1, self.xmax+1]
        yr = [self.ymin, sr.ymin, sr.ymax+1, self.ymax+1]
        zr = [self.zmin, sr.zmin, sr.zmax+1, self.zmax+1]

        for x in range(0, len(xr)-1):
            if xr[x] < xr[x+1]:
                for y in range(0, len(yr)-1):
                    if yr[y] < yr[y+1]:
                        for z in range(0, len(zr)-1):
                            if zr[z] < zr[z+1]:
                                piece = Range(True, xr[x], xr[x+1]-1, yr[y], yr[y+1]-1, zr[z], zr[z+1]-1)
                                if piece != sr:
                                    sub.append(piece)
        return sub
...
with open("input.txt") as file:
    ...
    for val,xyz in temp:
        xr, yr, zr = xyz.split(',')
        xmin, xmax = [int(x) for x in xr.strip('x=').split("..")]
        ymin, ymax = [int(y) for y in yr.strip('y=').split("..")]
